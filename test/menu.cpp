//menu.cpp
/*questions
	пункт меню 3 логика оставить только удалить??? --
	пункт меню 1 логика загрузка по имени и загрузить все --
	загрузить скрипт в git
	пункт меню 4 логика выводить файлы папки /firmware для удобства ввода пути или производльная папка ? --руками набирать да
	как получать IP, через вывод arduino, записывать руками или выгружать в облако (задание статических ip и имени) -- руками
	сразу рефакторить в класс -- да
	куда сохранять *.txt или разбирать sql, json -- вызываем функцию сохранения в 3 файла котнроллер связи и прошивки. вначале достаем данные и в конструкторы
	спросить про паттерн для меню -- нет
	!!!окончание по кнопке загрузить прошивку
*/
#include <iostream>
#include <string>
#include <map>
